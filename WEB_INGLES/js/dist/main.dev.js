...
window.addEventListener("scroll", callbackFunc); //FUNCION 9
...
function Validarcampos() {
  ...
  if (titulo == "" || nombre_apellido == "" || organizacion == "" || puesto == "" || email == "" || callen == "" || colonia == "" || codigop == "" || ciudad == "" || pais == "" || estado == "" || numtel == "" || textarea1 == "" && spamkill != "") {
    ...
    setTimeout(function cerrarmodal() {
...
function mostrarcontra() {
  ...
  var tipo = document.getElementById("txtContrasena"); // SE VALIDA SI EL CAMPO ES DE TIPO CONTRASEÑA

  if (tipo.type == "password") {
    // SI LO ES SE CAMBIARA DE TIPO CONTRASEÑA A TIPO TEXTO
    tipo.type = "text"; // AL SER DE TIPO TEXTO SE MOSTRARA EL ICONO DEL OJO A CERRAR 

    document.getElementById("eyeoff").style.display = "inline";
  } else {
    // SI NO SE REGRESARA A SER DE TIPO CONTRASEÑA 
    tipo.type = "password"; // SE DEJARA DE MOSTRAR EL OJO CON TACHE Y SE MOSTRARA EL OJO NORMAL 
...
function mostrarcontra2() {
  // SE CREA LA VARIABLE TIPO QUE CONTIENE EL VALOR DEL CAMPO DE CONTRASEÑA
  var tipo = document.getElementById("txtContrasena_Confirmacion"); // SE VALIDA SI EL CAMPO ES DE TIPO CONTRASEÑA
...
function verdad() {
  if (document.getElementById("Recordar").checked == true) {
    document.getElementById("Recordar").value = true; // SI NO ES VERDAD EL VALOR DEL CHECK SERA FALSO   
  } else {
