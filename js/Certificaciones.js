$(document).ready(function(){

    document.getElementById("contenedorletrap").innerHTML = `
    <div class="divfooter2 center-align"><span class="inicial--productos Tipografia--general"><b>All</b></span></div><br><br><br></div>
    `
    document.getElementById("contenedorliteral").innerHTML = `
    
   
            <table class="compliance_tab">
                <tbody>

                


                    <tr>
                        <td class="letranegro Tipografia--general General--Cuerpo"
                            style="background-color:#000020;color:white;text-align:center" colspan="3">Certificaciones
                            </td>
                    </tr>

                    <tr class="border_compliance_tabla">
                <td class="letranegro Tipografia--general General--Cuerpo"
                    style="width:33.33%;text-align:center"><b style="color:#000020">AIB International</b></td>
                <td class="letranegro Tipografia--general General--Cuerpo"
                    style="width:765px;text-align:justify;">
                    <p style="text-align: justify;padding:5px;width:765px;">AIB International es una certificación que capacita al personal de las empresas para llevar sus instalaciones a los estándares de seguridad y limpieza reconocidos internacionalmente. Esta Certificación sirve para garantizar que las instalaciones donde se fabrican y envasan los productos que comercializamos, cumplan con rigurosos estándares de Seguridad y Limpieza
                        <span>Para más información</span> <a href="https://www.aibinternational.com/es/"
                            style="color:rgba(2 146 238 /99%);">clic aquí</a></p>
                </td>
                <td class="letranegro Tipografia--general General--Cuerpo"
                    style="width:50%;text-align:center"><img src="img/AIB-INTERNATIONAL_IMAGE.png" alt=""
                        style="width:50%;"></td>
            </tr>


            <tr class="border_compliance_tabla">
                        <td class="letranegro Tipografia--general General--Cuerpo"
                            style="width:33.33%;text-align:center"><b style="color:#000020">BRC</b></td>
                        <td class="letranegro Tipografia--general General--Cuerpo"
                            style="width:765px;text-align:justify;">
                            <p style="text-align: justify;padding:5px;width:765px;">La norma BRC es reconocida por la Iniciativa mundial de Seguridad Alimentaria (GFSI), programa destinado a armonizar las normas internacionales de seguridad alimentaria, velando que cumplan con los requisitos que garantizan la seguridad, calidad, y legalidad de sus alimentos.
                                <span>Para más información</span> <a href="https://www.normas-iso.com/brc-certificacion-global-seguridad-alimentaria/"
                                    style="color:rgba(2 146 238 /99%);">clic aquí</a></p>
                        </td>
                        <td class="letranegro Tipografia--general General--Cuerpo"
                            style="width:100%;text-align:center"><img src="img/BCR_IMAGE.png" alt=""
                                style="width:38%;"></td>
                    </tr>


                    <tr class="border_compliance_tabla">
                    <td class="letranegro Tipografia--general General--Cuerpo"
                        style="width:33.33%;text-align:center"><b style="color:#000020">Ecocert</b></td>
                    <td class="letranegro Tipografia--general General--Cuerpo"
                        style="width:765px;text-align:justify">
                        <p style="text-align: justify;padding:5px;width:765px;">El Certificado ECOCERT es la garantía para un producto en que cumpla con todos los requisitos de sostenibilidad, desde la producción de sus materias primas hasta el producto terminado. <span>Para más información</span> <a
                                href="https://www.ecocert.com/es-MX/quienes-somos"
                                style="color:rgba(2 146 238 /99%);">clic aquí</a></p>
                    </td>
                    <td class="letranegro Tipografia--general General--Cuerpo"
                        style="width:33.33%;text-align:center"><img src="img/ECO-CERT_IMAGE.png" alt=""
                            style="width:50%;"></td>
                </tr>

                <tr class="border_compliance_tabla">
                <td class="letranegro Tipografia--general General--Cuerpo"
                    style="width:33.33%;text-align:center"><b style="color:#000020">FCC</b></td>
                <td class="letranegro Tipografia--general General--Cuerpo"
                    style="width:765px;text-align:justify;">
                    <p style="text-align: justify;padding:5px;width:765px;">El Códice de Sustancias Químicas para Alimentos o Food Chemicals Codex (FCC) es una colección de estándares internacionalmente reconocidos para la pureza e identidad de los ingredientes alimentarios. <span>Para más
                            información</span> <a href="https://www.foodchemicalscodex.org/"
                            style="color:rgba(2 146 238 /99%);">clic aquí</a></p>
                </td>


                <td class="letranegro Tipografia--general General--Cuerpo"
                    style="width:33.33%;text-align:center"><img
                        src="img/FCC_IMAGE.png" alt="" style="width:50%;">
                </td>
            </tr>

            <tr class="border_compliance_tabla">
            <td class="letranegro Tipografia--general General--Cuerpo"
                style="width:33.33%;text-align:center"><b style="color:#000020">FSSC 22000</b></td>
            <td class="letranegro Tipografia--general General--Cuerpo"
                style="width:765px;text-align:justify;">
                <p style="text-align: justify;padding:5px;width:765px;">La Norma FSSC 22000 establece un marco de referencia para los requisitos de la seguridad alimentaria incorporando buenas prácticas en los sistemas agrícolas, de fabricación y distribución, y considerando los principios para el análisis de peligros y puntos críticos de control. <span>Para más
                        información</span> <a href="https://www.dnv.com.mx/services/fssc-22000-certificacion-en-seguridad-alimentaria-5161#:~:text=%C2%BFQu%C3%A9%20es%20la%20FSSC%2022000,para%20la%20fabricaci%C3%B3n%20de%20envases.php"
                        style="color:rgba(2 146 238 /99%);">clic aquí</a></p>
            </td>
            <td class="letranegro Tipografia--general General--Cuerpo"
                style="width:33.33%;text-align:center"><img src="img/FSSC-22000_IMAGE.jpg" alt=""
                    style="width:50%;"></td>
        </tr>

        <tr class="border_compliance_tabla">
                        <td class="letranegro Tipografia--general General--Cuerpo"
                            style="width:33.33%;text-align:center"><b style="color:#000020">GMO</b></td>
                        <td class="letranegro Tipografia--general General--Cuerpo"
                            style="width:765px;text-align:justify;">
                            <p style="text-align: justify;padding:5px;width:765px;">Los Organismos Modificados Genéticamente (GMO) son plantas y animales creados en los laboratorios al insertar genes de una especie en la secuencia de ADN de otra especie <span>Para más información</span> <a
                                    href="https://www.genome.gov/es/genetics-glossary/Genetically-Modified-Organism"
                                    style="color:rgba(2 146 238 /99%);">clic aquí</a></p>
                        </td>
                        <td class="letranegro Tipografia--general General--Cuerpo"
                            style="width:33.33%;text-align:center"><img src="img/GMO_NEW_LOGO.jpg" alt=""
                                style="width:50%;"></td>
                    </tr>


                  
                    <tr class="border_compliance_tabla">
                        <td class="letranegro Tipografia--general General--Cuerpo"
                            style="width:33.33%;text-align:center"><b style="color:#000020">GMP ICH Q7</b></td>
                        <td class="letranegro Tipografia--general General--Cuerpo"
                            style="width:765px;text-align:justify">
                            <p style="text-align: justify;padding:5px;width:765px;">Buenas prácticas de fabricación de principios activos farmacéuticos
                            (Good ManufacturingPractices for Active Pharmaceutical Ingredients) es un documento que proporciona orientación sobrelas buenas prácticas de fabricación para la fabricación de principios activos farmacéuticos (API porsus siglas en inglés) bajo un sistema adecuado de gestión de la calidad. De mismo modo pretendeayudar a garantizar que los API cumplan los requisitos de calidad y pureza.<span>Para más
                                    información</span> <a
                                    href="Proximamente.php"
                                    style="color:rgba(2 146 238 /99%);">clic aquí</a></p>
                        </td>
                        <td class="letranegro Tipografia--general General--Cuerpo"
                            style="width:33.33%;text-align:center"><img src="img/CB_ICH_Q7_271123.jpg" alt=""
                                style="width:70%;"></td>
                    </tr>

                    <tr class="border_compliance_tabla">
                        <td class="letranegro Tipografia--general General--Cuerpo"
                            style="width:33.33%;text-align:center"><b style="color:#000020">HACCP</b></td>
                        <td class="letranegro Tipografia--general General--Cuerpo"
                            style="width:765px;text-align:justify;">
                            <p style="text-align: justify;padding:5px;width:765px;">El Sistema de Análisis de Peligros y Control de Puntos Críticos (HACCP - Hazard Análisis and Critical Control Points) el cual permite identificar peligros específicos y medidas para su control con el fin de garantizar la inocuidad de los alimentos. <span>Para más información</span> <a
                                    href="https://haccp-international.com/" style="color:rgba(2 146 238 /99%);">clic aquí</a></p>
                        </td>
                        <td class="letranegro Tipografia--general General--Cuerpo"
                            style="width:33.33%;text-align:center"><img src="img/HACCP_NEW_LOGO.jpg" alt="" style="width:50%;"></td>
                    </tr>

                    <tr class="border_compliance_tabla">
                        <td class="letranegro Tipografia--general General--Cuerpo"
                            style="width:33.33%;text-align:center"><b style="color:#000020">HALAL</b></td>
                        <td class="letranegro Tipografia--general General--Cuerpo"
                            style="width:765px;text-align:justify;">
                            <p style="text-align: justify;padding:5px;width:765px;">La Certificación Halal determina que un producto o servicio cumple con lo que establece Ley Islámica (Shaira) para que este sea consumido, esto garantiza que los productos agroalimentarios cumplen con el conjunto de prácticas permitidas por la religión musulmana. El término Halal se refiere a todo aquello que está permitido, aquello que es benéfico y saludable para el ser humano ya que aporta mejoras en la calidad de vida y reduce riesgos para la salud.
                                <span>Para más información</span> <a href="https://www.institutohalal.com/que-es-halal/"
                                    style="color:rgba(2 146 238 /99%);">clic aquí</a></p>
                        </td>
                        <td class="letranegro Tipografia--general General--Cuerpo"
                            style="width:33.33%;text-align:center"><img src="img/HALAL_IMAGE.png" alt=""
                                style="width:50%;"></td>
                    </tr>


                    <tr class="border_compliance_tabla">
                        <td class="letranegro Tipografia--general General--Cuerpo"
                            style="width:33.33%;text-align:center"><b style="color:#000020">ISO 27001</b></td>
                        <td class="letranegro Tipografia--general General--Cuerpo"
                            style="width:765px;text-align:justify">
                            <p style="text-align: justify;padding:5px;width:765px;">Sistema de Gestión de Seguridad de la Información
                            establece parámetros de administración dela información a través del conjunto de políticas, procedimientos y directrices, apalancado de recursosy actividades asociados, que son administrados colectivamente por una empresa u organización,prioritariamente busca proteger sus activos de información esenciales.<span>Para más información</span> <a
                                    href="Proximamente.php"
                                    style="color:rgba(2 146 238 /99%);">clic aquí</a></p>
                        </td>
                        <td class="letranegro Tipografia--general General--Cuerpo"
                            style="width:33.33%;text-align:center"><img src="img/iso-27001_IMAGE.png" alt=""
                                style="width:80%;"></td>
                    </tr>

                   
                  
                    
                    
                    <tr class="border_compliance_tabla">
                        <td class="letranegro Tipografia--general General--Cuerpo"
                            style="width:33.33%;text-align:center"><b style="color:#000020">ISO 37001</b></td>
                        <td class="letranegro Tipografia--general General--Cuerpo"
                            style="width:765px;text-align:justify;">
                            <p style="text-align: justify;padding:5px;width:765px;">Sistema de Gestión Antisoborno
                            es un sistema que permite prevenir, detectar y mitigarcualquier intento o acto de soborno. El sistema busca establecer, implementar, mantener y mejorarcontinuamente los elementos que lleven a mitigar el riesgo de soborno en las empresas uorganizaciones.
                                <span>Para más información</span> <a href="Proximamente.php"
                                    style="color:rgba(2 146 238 /99%);">clic aquí</a></p>
                        </td>
                        <td class="letranegro Tipografia--general General--Cuerpo"
                            style="width:33.33%;text-align:center"><img src="img/ISO-37001_IMAGE.jpg" alt=""
                                style="width:70%;"></td>
                    </tr>

                    
                    <tr class="border_compliance_tabla">
                    <td class="letranegro Tipografia--general General--Cuerpo"
                        style="width:33.33%;text-align:center"><b style="color:#000020">ISO 37301</b></td>
                    <td class="letranegro Tipografia--general General--Cuerpo"
                        style="width:765px;text-align:justify;">
                        <p style="text-align: justify;padding:5px;width:765px;">LUn
                        Sistema de Gestión de Compliance
                        se compone de todas las medidas, estructuras y procesosque una empresa u organización debe establecer para garantizar el cumplimiento de las normasaplicables y la ley vigente. De mismo modo, debe ser conocido y apoyado por la alta dirección, paramantener desde todas las áreas la identidad e integridad corporativa.
                            <span>Para más información</span> <a href="Proximamente.php"
                                style="color:rgba(2 146 238 /99%);">clic aquí</a></p>
                    </td>
                    <td class="letranegro Tipografia--general General--Cuerpo"
                        style="width:33.33%;text-align:center"><img src="img/ISO-37301_IMAGE.jpg" alt=""
                            style="width:100%;"></td>
                </tr>

                <tr class="border_compliance_tabla">
                <td class="letranegro Tipografia--general General--Cuerpo"
                    style="width:33.33%;text-align:center"><b style="color:#000020">ISO 9001</b></td>
                <td class="letranegro Tipografia--general General--Cuerpo"
                    style="width:765px;text-align:justify;">
                    <p style="text-align: justify;padding:5px;width:765px;">La Certificación ISO 9001:2015 es una norma internacional de gestión de la calidad aplicable a cualquier tipo de organización en cualquier sector o actividad empresarial o industrial. Está basada en los siete principios de gestión de calidad, fundamentales para una buena gestión empresarial. 
                        <span>Para más información</span> <a href="https://www.iso.org/obp/ui/#iso:std:iso:9001:ed-4:v2:es"
                            style="color:rgba(2 146 238 /99%);">clic aquí</a></p>
                </td>
                <td class="letranegro Tipografia--general General--Cuerpo"
                    style="width:33.33%;text-align:center"><img src="img/ISO-9001_IMAGE.jpg" alt=""
                        style="width:70%;"></td>
            </tr>
                    

            <tr class="border_compliance_tabla">
            <td class="letranegro Tipografia--general General--Cuerpo"
                style="width:33.33%;text-align:center"><b style="color:#000020">KOSHER</b></td>
            <td class="letranegro Tipografia--general General--Cuerpo"
                style="width:765px;text-align:justify;">
                <p style="text-align: justify;padding:5px;width:765px;">La Certificación Kosher es aquella que indica que dichos productos respetan los preceptos de la religión judía y que, por tanto, se consideran puros y aptos para ser ingeridos por los practicantes de dicha religión.
                    <span>Para más información</span> <a href="https://oukosher.org/espanol/que-es-el-kosher/"
                        style="color:rgba(2 146 238 /99%);">clic aquí</a></p>
            </td>
            <td class="letranegro Tipografia--general General--Cuerpo"
                style="width:33.33%;text-align:center"><img src="img/KOSHER_IMAGE.jpg" alt=""
                    style="width:50%;"></td>
        </tr>


        <tr class="border_compliance_tabla">
        <td class="letranegro Tipografia--general General--Cuerpo"
            style="width:33.33%;text-align:center"><b style="color:#000020">NON-GMO</b></td>
        <td class="letranegro Tipografia--general General--Cuerpo"
            style="width:765px;text-align:justify;">
            <p style="text-align: justify;padding:5px;width:765px;">La certificación NON GMO evalúa alimentos de cuyos ingredientes o algún algún proceso de la creación de ellos, se ocupen organismos modificados genéticamente. Dicho de otro modo, certifica productos alimenticios completamente orgánicos, con todas sus cualidades naturales intactas.
                <span>Para más información</span> <a href="Proximamente.php"
                    style="color:rgba(2 146 238 /99%);">clic aquí</a></p>
        </td>
        <td class="letranegro Tipografia--general General--Cuerpo"
            style="width:33.33%;text-align:center"><img src="img/NON-GMO_IMAGE.jpg" alt=""
                style="width:50%;"></td>
    </tr>

    <tr class="border_compliance_tabla">
    <td class="letranegro Tipografia--general General--Cuerpo"
        style="width:33.33%;text-align:center"><b style="color:#000020">PAREVE</b></td>
    <td class="letranegro Tipografia--general General--Cuerpo"
        style="width:765px;text-align:justify;">
        <p style="text-align: justify;padding:5px;width:765px;">La certificación
        Kosher PAREVE
        (NEUTRO) cumple al pie de la letra con las exigencias de aquellaspersonas que por diversas razones requieren de una dieta vegetariana, vegana o muy estricta, y queno pueden contener ningún tipo de grasa animal, debido a que esta certificación asegura lainexistencia de contaminación o arrastre de partículas que afectan a personas con diferentes tipos dealergias, principalmente de derivados de lácteos o cárnicos.
            <span>Para más información</span> <a href="https://ukkosher.org/empresas/carnicos-lacteos-y-pareve/#:~:text=Los%20alimentos%20que%20no%20son,caf%C3%A9%2C%20t%C3%A9%20y%20diversos%20dulces."
                style="color:rgba(2 146 238 /99%);">clic aquí</a></p>
    </td>
    <td class="letranegro Tipografia--general General--Cuerpo"
        style="width:33.33%;text-align:center"><img src="img/CB_PAREVE_271123.png" alt=""
            style="width:50%;"></td>
</tr>


<tr class="border_compliance_tabla">
<td class="letranegro Tipografia--general General--Cuerpo"
    style="width:33.33%;text-align:center"><b style="color:#000020">SQF</b></td>
<td class="letranegro Tipografia--general General--Cuerpo"
    style="width:765px;text-align:justify;">
    <p style="text-align: justify;padding:5px;width:765px;">El programa SQF (Safe Quality Food / Seguridad de Calidad Alimentaria) es un programa de calidad e inocuidad de los alimentos riguroso y confiable con reconocimiento por parte de minoristas, propietarios de marcas y proveedores de servicios de alimentos a nivel mundial.
        <span>Para más información</span> <a href="https://www.sqfi.com/es-mx/our-program/what-is-sqf/noriko-nakama"
            style="color:rgba(2 146 238 /99%);">clic aquí</a></p>
</td>
<td class="letranegro Tipografia--general General--Cuerpo"
    style="width:33.33%;text-align:center"><img src="img/SQF_IMAGE.png" alt=""
        style="width:50%;"></td>
</tr>



                </tbody>
            </table>

 

    `


});


function filtroall(){
    
    document.getElementById("contenedorletrap").innerHTML = `
    <div class="divfooter2 center-align"><span class="inicial--productos Tipografia--general"><b>All</b></span></div><br><br><br></div>
    `
    document.getElementById("contenedorliteral").innerHTML = `



    <table class="compliance_tab">
    <tbody>

    


        <tr>
            <td class="letranegro Tipografia--general General--Cuerpo"
                style="background-color:#000020;color:white;text-align:center" colspan="3">Certificaciones
                </td>
        </tr>

        <tr class="border_compliance_tabla">
    <td class="letranegro Tipografia--general General--Cuerpo"
        style="width:33.33%;text-align:center"><b style="color:#000020">AIB International</b></td>
    <td class="letranegro Tipografia--general General--Cuerpo"
        style="width:765px;text-align:justify;">
        <p style="text-align: justify;padding:5px;width:765px;">AIB International es una certificación que capacita al personal de las empresas para llevar sus instalaciones a los estándares de seguridad y limpieza reconocidos internacionalmente. Esta Certificación sirve para garantizar que las instalaciones donde se fabrican y envasan los productos que comercializamos, cumplan con rigurosos estándares de Seguridad y Limpieza
            <span>Para más información</span> <a href="https://www.aibinternational.com/es/"
                style="color:rgba(2 146 238 /99%);">clic aquí</a></p>
    </td>
    <td class="letranegro Tipografia--general General--Cuerpo"
        style="width:50%;text-align:center"><img src="img/AIB-INTERNATIONAL_IMAGE.png" alt=""
            style="width:50%;"></td>
</tr>


<tr class="border_compliance_tabla">
            <td class="letranegro Tipografia--general General--Cuerpo"
                style="width:33.33%;text-align:center"><b style="color:#000020">BRC</b></td>
            <td class="letranegro Tipografia--general General--Cuerpo"
                style="width:765px;text-align:justify;">
                <p style="text-align: justify;padding:5px;width:765px;">La norma BRC es reconocida por la Iniciativa mundial de Seguridad Alimentaria (GFSI), programa destinado a armonizar las normas internacionales de seguridad alimentaria, velando que cumplan con los requisitos que garantizan la seguridad, calidad, y legalidad de sus alimentos.
                    <span>Para más información</span> <a href="https://www.normas-iso.com/brc-certificacion-global-seguridad-alimentaria/"
                        style="color:rgba(2 146 238 /99%);">clic aquí</a></p>
            </td>
            <td class="letranegro Tipografia--general General--Cuerpo"
                style="width:100%;text-align:center"><img src="img/BCR_IMAGE.png" alt=""
                    style="width:38%;"></td>
        </tr>


        <tr class="border_compliance_tabla">
        <td class="letranegro Tipografia--general General--Cuerpo"
            style="width:33.33%;text-align:center"><b style="color:#000020">Ecocert</b></td>
        <td class="letranegro Tipografia--general General--Cuerpo"
            style="width:765px;text-align:justify">
            <p style="text-align: justify;padding:5px;width:765px;">El Certificado ECOCERT es la garantía para un producto en que cumpla con todos los requisitos de sostenibilidad, desde la producción de sus materias primas hasta el producto terminado. <span>Para más información</span> <a
                    href="https://www.ecocert.com/es-MX/quienes-somos"
                    style="color:rgba(2 146 238 /99%);">clic aquí</a></p>
        </td>
        <td class="letranegro Tipografia--general General--Cuerpo"
            style="width:33.33%;text-align:center"><img src="img/ECO-CERT_IMAGE.png" alt=""
                style="width:50%;"></td>
    </tr>

    <tr class="border_compliance_tabla">
    <td class="letranegro Tipografia--general General--Cuerpo"
        style="width:33.33%;text-align:center"><b style="color:#000020">FCC</b></td>
    <td class="letranegro Tipografia--general General--Cuerpo"
        style="width:765px;text-align:justify;">
        <p style="text-align: justify;padding:5px;width:765px;">El Códice de Sustancias Químicas para Alimentos o Food Chemicals Codex (FCC) es una colección de estándares internacionalmente reconocidos para la pureza e identidad de los ingredientes alimentarios. <span>Para más
                información</span> <a href="https://www.foodchemicalscodex.org/"
                style="color:rgba(2 146 238 /99%);">clic aquí</a></p>
    </td>


    <td class="letranegro Tipografia--general General--Cuerpo"
        style="width:33.33%;text-align:center"><img
            src="img/FCC_IMAGE.png" alt="" style="width:50%;">
    </td>
</tr>

<tr class="border_compliance_tabla">
<td class="letranegro Tipografia--general General--Cuerpo"
    style="width:33.33%;text-align:center"><b style="color:#000020">FSSC 22000</b></td>
<td class="letranegro Tipografia--general General--Cuerpo"
    style="width:765px;text-align:justify;">
    <p style="text-align: justify;padding:5px;width:765px;">La Norma FSSC 22000 establece un marco de referencia para los requisitos de la seguridad alimentaria incorporando buenas prácticas en los sistemas agrícolas, de fabricación y distribución, y considerando los principios para el análisis de peligros y puntos críticos de control. <span>Para más
            información</span> <a href="https://www.dnv.com.mx/services/fssc-22000-certificacion-en-seguridad-alimentaria-5161#:~:text=%C2%BFQu%C3%A9%20es%20la%20FSSC%2022000,para%20la%20fabricaci%C3%B3n%20de%20envases.php"
            style="color:rgba(2 146 238 /99%);">clic aquí</a></p>
</td>
<td class="letranegro Tipografia--general General--Cuerpo"
    style="width:33.33%;text-align:center"><img src="img/FSSC-22000_IMAGE.jpg" alt=""
        style="width:50%;"></td>
</tr>

<tr class="border_compliance_tabla">
            <td class="letranegro Tipografia--general General--Cuerpo"
                style="width:33.33%;text-align:center"><b style="color:#000020">GMO</b></td>
            <td class="letranegro Tipografia--general General--Cuerpo"
                style="width:765px;text-align:justify;">
                <p style="text-align: justify;padding:5px;width:765px;">Los Organismos Modificados Genéticamente (GMO) son plantas y animales creados en los laboratorios al insertar genes de una especie en la secuencia de ADN de otra especie <span>Para más información</span> <a
                        href="https://www.genome.gov/es/genetics-glossary/Genetically-Modified-Organism"
                        style="color:rgba(2 146 238 /99%);">clic aquí</a></p>
            </td>
            <td class="letranegro Tipografia--general General--Cuerpo"
                style="width:33.33%;text-align:center"><img src="img/GMO_NEW_LOGO.jpg" alt=""
                    style="width:50%;"></td>
        </tr>


      
        <tr class="border_compliance_tabla">
            <td class="letranegro Tipografia--general General--Cuerpo"
                style="width:33.33%;text-align:center"><b style="color:#000020">GMP ICH Q7</b></td>
            <td class="letranegro Tipografia--general General--Cuerpo"
                style="width:765px;text-align:justify">
                <p style="text-align: justify;padding:5px;width:765px;">Buenas prácticas de fabricación de principios activos farmacéuticos
                (Good ManufacturingPractices for Active Pharmaceutical Ingredients) es un documento que proporciona orientación sobrelas buenas prácticas de fabricación para la fabricación de principios activos farmacéuticos (API porsus siglas en inglés) bajo un sistema adecuado de gestión de la calidad. De mismo modo pretendeayudar a garantizar que los API cumplan los requisitos de calidad y pureza.<span>Para más
                        información</span> <a
                        href="Proximamente.php"
                        style="color:rgba(2 146 238 /99%);">clic aquí</a></p>
            </td>
            <td class="letranegro Tipografia--general General--Cuerpo"
                style="width:33.33%;text-align:center"><img src="img/CB_ICH_Q7_271123.jpg" alt=""
                    style="width:70%;"></td>
        </tr>

        <tr class="border_compliance_tabla">
            <td class="letranegro Tipografia--general General--Cuerpo"
                style="width:33.33%;text-align:center"><b style="color:#000020">HACCP</b></td>
            <td class="letranegro Tipografia--general General--Cuerpo"
                style="width:765px;text-align:justify;">
                <p style="text-align: justify;padding:5px;width:765px;">El Sistema de Análisis de Peligros y Control de Puntos Críticos (HACCP - Hazard Análisis and Critical Control Points) el cual permite identificar peligros específicos y medidas para su control con el fin de garantizar la inocuidad de los alimentos. <span>Para más información</span> <a
                        href="https://haccp-international.com/" style="color:rgba(2 146 238 /99%);">clic aquí</a></p>
            </td>
            <td class="letranegro Tipografia--general General--Cuerpo"
                style="width:33.33%;text-align:center"><img src="img/HACCP_NEW_LOGO.jpg" alt="" style="width:50%;"></td>
        </tr>

        <tr class="border_compliance_tabla">
            <td class="letranegro Tipografia--general General--Cuerpo"
                style="width:33.33%;text-align:center"><b style="color:#000020">HALAL</b></td>
            <td class="letranegro Tipografia--general General--Cuerpo"
                style="width:765px;text-align:justify;">
                <p style="text-align: justify;padding:5px;width:765px;">La Certificación Halal determina que un producto o servicio cumple con lo que establece Ley Islámica (Shaira) para que este sea consumido, esto garantiza que los productos agroalimentarios cumplen con el conjunto de prácticas permitidas por la religión musulmana. El término Halal se refiere a todo aquello que está permitido, aquello que es benéfico y saludable para el ser humano ya que aporta mejoras en la calidad de vida y reduce riesgos para la salud.
                    <span>Para más información</span> <a href="https://www.institutohalal.com/que-es-halal/"
                        style="color:rgba(2 146 238 /99%);">clic aquí</a></p>
            </td>
            <td class="letranegro Tipografia--general General--Cuerpo"
                style="width:33.33%;text-align:center"><img src="img/HALAL_IMAGE.png" alt=""
                    style="width:50%;"></td>
        </tr>


        <tr class="border_compliance_tabla">
            <td class="letranegro Tipografia--general General--Cuerpo"
                style="width:33.33%;text-align:center"><b style="color:#000020">ISO 27001</b></td>
            <td class="letranegro Tipografia--general General--Cuerpo"
                style="width:765px;text-align:justify">
                <p style="text-align: justify;padding:5px;width:765px;">Sistema de Gestión de Seguridad de la Información
                establece parámetros de administración dela información a través del conjunto de políticas, procedimientos y directrices, apalancado de recursosy actividades asociados, que son administrados colectivamente por una empresa u organización,prioritariamente busca proteger sus activos de información esenciales.<span>Para más información</span> <a
                        href="Proximamente.php"
                        style="color:rgba(2 146 238 /99%);">clic aquí</a></p>
            </td>
            <td class="letranegro Tipografia--general General--Cuerpo"
                style="width:33.33%;text-align:center"><img src="img/iso-27001_IMAGE.png" alt=""
                    style="width:80%;"></td>
        </tr>

       
      
        
        
        <tr class="border_compliance_tabla">
            <td class="letranegro Tipografia--general General--Cuerpo"
                style="width:33.33%;text-align:center"><b style="color:#000020">ISO 37001</b></td>
            <td class="letranegro Tipografia--general General--Cuerpo"
                style="width:765px;text-align:justify;">
                <p style="text-align: justify;padding:5px;width:765px;">Sistema de Gestión Antisoborno
                es un sistema que permite prevenir, detectar y mitigarcualquier intento o acto de soborno. El sistema busca establecer, implementar, mantener y mejorarcontinuamente los elementos que lleven a mitigar el riesgo de soborno en las empresas uorganizaciones.
                    <span>Para más información</span> <a href="Proximamente.php"
                        style="color:rgba(2 146 238 /99%);">clic aquí</a></p>
            </td>
            <td class="letranegro Tipografia--general General--Cuerpo"
                style="width:33.33%;text-align:center"><img src="img/ISO-37001_IMAGE.jpg" alt=""
                    style="width:70%;"></td>
        </tr>

        
        <tr class="border_compliance_tabla">
        <td class="letranegro Tipografia--general General--Cuerpo"
            style="width:33.33%;text-align:center"><b style="color:#000020">ISO 37301</b></td>
        <td class="letranegro Tipografia--general General--Cuerpo"
            style="width:765px;text-align:justify;">
            <p style="text-align: justify;padding:5px;width:765px;">LUn
            Sistema de Gestión de Compliance
            se compone de todas las medidas, estructuras y procesosque una empresa u organización debe establecer para garantizar el cumplimiento de las normasaplicables y la ley vigente. De mismo modo, debe ser conocido y apoyado por la alta dirección, paramantener desde todas las áreas la identidad e integridad corporativa.
                <span>Para más información</span> <a href="Proximamente.php"
                    style="color:rgba(2 146 238 /99%);">clic aquí</a></p>
        </td>
        <td class="letranegro Tipografia--general General--Cuerpo"
            style="width:33.33%;text-align:center"><img src="img/ISO-37301_IMAGE.jpg" alt=""
                style="width:100%;"></td>
    </tr>

    <tr class="border_compliance_tabla">
    <td class="letranegro Tipografia--general General--Cuerpo"
        style="width:33.33%;text-align:center"><b style="color:#000020">ISO 9001</b></td>
    <td class="letranegro Tipografia--general General--Cuerpo"
        style="width:765px;text-align:justify;">
        <p style="text-align: justify;padding:5px;width:765px;">La Certificación ISO 9001:2015 es una norma internacional de gestión de la calidad aplicable a cualquier tipo de organización en cualquier sector o actividad empresarial o industrial. Está basada en los siete principios de gestión de calidad, fundamentales para una buena gestión empresarial. 
            <span>Para más información</span> <a href="https://www.iso.org/obp/ui/#iso:std:iso:9001:ed-4:v2:es"
                style="color:rgba(2 146 238 /99%);">clic aquí</a></p>
    </td>
    <td class="letranegro Tipografia--general General--Cuerpo"
        style="width:33.33%;text-align:center"><img src="img/ISO-9001_IMAGE.jpg" alt=""
            style="width:70%;"></td>
</tr>
        

<tr class="border_compliance_tabla">
<td class="letranegro Tipografia--general General--Cuerpo"
    style="width:33.33%;text-align:center"><b style="color:#000020">KOSHER</b></td>
<td class="letranegro Tipografia--general General--Cuerpo"
    style="width:765px;text-align:justify;">
    <p style="text-align: justify;padding:5px;width:765px;">La Certificación Kosher es aquella que indica que dichos productos respetan los preceptos de la religión judía y que, por tanto, se consideran puros y aptos para ser ingeridos por los practicantes de dicha religión.
        <span>Para más información</span> <a href="https://oukosher.org/espanol/que-es-el-kosher/"
            style="color:rgba(2 146 238 /99%);">clic aquí</a></p>
</td>
<td class="letranegro Tipografia--general General--Cuerpo"
    style="width:33.33%;text-align:center"><img src="img/KOSHER_IMAGE.jpg" alt=""
        style="width:50%;"></td>
</tr>


<tr class="border_compliance_tabla">
<td class="letranegro Tipografia--general General--Cuerpo"
style="width:33.33%;text-align:center"><b style="color:#000020">NON-GMO</b></td>
<td class="letranegro Tipografia--general General--Cuerpo"
style="width:765px;text-align:justify;">
<p style="text-align: justify;padding:5px;width:765px;">La certificación NON GMO evalúa alimentos de cuyos ingredientes o algún algún proceso de la creación de ellos, se ocupen organismos modificados genéticamente. Dicho de otro modo, certifica productos alimenticios completamente orgánicos, con todas sus cualidades naturales intactas.
    <span>Para más información</span> <a href="Proximamente.php"
        style="color:rgba(2 146 238 /99%);">clic aquí</a></p>
</td>
<td class="letranegro Tipografia--general General--Cuerpo"
style="width:33.33%;text-align:center"><img src="img/NON-GMO_IMAGE.jpg" alt=""
    style="width:50%;"></td>
</tr>

<tr class="border_compliance_tabla">
<td class="letranegro Tipografia--general General--Cuerpo"
style="width:33.33%;text-align:center"><b style="color:#000020">PAREVE</b></td>
<td class="letranegro Tipografia--general General--Cuerpo"
style="width:765px;text-align:justify;">
<p style="text-align: justify;padding:5px;width:765px;">La certificación
Kosher PAREVE
(NEUTRO) cumple al pie de la letra con las exigencias de aquellaspersonas que por diversas razones requieren de una dieta vegetariana, vegana o muy estricta, y queno pueden contener ningún tipo de grasa animal, debido a que esta certificación asegura lainexistencia de contaminación o arrastre de partículas que afectan a personas con diferentes tipos dealergias, principalmente de derivados de lácteos o cárnicos.
<span>Para más información</span> <a href="https://ukkosher.org/empresas/carnicos-lacteos-y-pareve/#:~:text=Los%20alimentos%20que%20no%20son,caf%C3%A9%2C%20t%C3%A9%20y%20diversos%20dulces."
    style="color:rgba(2 146 238 /99%);">clic aquí</a></p>
</td>
<td class="letranegro Tipografia--general General--Cuerpo"
style="width:33.33%;text-align:center"><img src="img/CB_PAREVE_271123.png" alt=""
style="width:50%;"></td>
</tr>


<tr class="border_compliance_tabla">
<td class="letranegro Tipografia--general General--Cuerpo"
style="width:33.33%;text-align:center"><b style="color:#000020">SQF</b></td>
<td class="letranegro Tipografia--general General--Cuerpo"
style="width:765px;text-align:justify;">
<p style="text-align: justify;padding:5px;width:765px;">El programa SQF (Safe Quality Food / Seguridad de Calidad Alimentaria) es un programa de calidad e inocuidad de los alimentos riguroso y confiable con reconocimiento por parte de minoristas, propietarios de marcas y proveedores de servicios de alimentos a nivel mundial.
<span>Para más información</span> <a href="https://www.sqfi.com/es-mx/our-program/what-is-sqf/noriko-nakama"
style="color:rgba(2 146 238 /99%);">clic aquí</a></p>
</td>
<td class="letranegro Tipografia--general General--Cuerpo"
style="width:33.33%;text-align:center"><img src="img/SQF_IMAGE.png" alt=""
style="width:50%;"></td>
</tr>



    </tbody>
</table>


    
    `
};


function filtroa(){
    document.getElementById("contenedorletrap").innerHTML = `
    <div class="divfooter2" style="text-align:center;"><span style="font-size:55px;border-bottom:3px solid #262137;"><b>A</b></span></div><br><br></div>
    `
    document.getElementById("contenedorliteral").innerHTML = `

   
    <table class="compliance_tab">
    <tbody>

    <tr>
    <td class="letranegro Tipografia--general General--Cuerpo"
        style="background-color:#000020;color:white;text-align:center" colspan="3">Certificaciones
        </td>
</tr>

<tr class="border_compliance_tabla">
<td class="letranegro Tipografia--general General--Cuerpo"
style="width:33.33%;text-align:center"><b style="color:#000020">AIB International</b></td>
<td class="letranegro Tipografia--general General--Cuerpo"
style="width:765px;text-align:justify;">
<p style="text-align: justify;padding:5px;width:765px;">AIB International es una certificación que capacita al personal de las empresas para llevar sus instalaciones a los estándares de seguridad y limpieza reconocidos internacionalmente. Esta Certificación sirve para garantizar que las instalaciones donde se fabrican y envasan los productos que comercializamos, cumplan con rigurosos estándares de Seguridad y Limpieza
    <span>Para más información</span> <a href="https://www.aibinternational.com/es/"
        style="color:rgba(2 146 238 /99%);">clic aquí</a></p>
</td>
<td class="letranegro Tipografia--general General--Cuerpo"
style="width:50%;text-align:center"><img src="img/AIB-INTERNATIONAL_IMAGE.png" alt=""
    style="width:50%;"></td>
</tr>
   


</table>
</tbody>

   

    `
}

function filtrob(){
    document.getElementById("contenedorletrap").innerHTML = `
    <div class="divfooter2" style="text-align:center;"><span style="font-size:55px;border-bottom:3px solid #262137;"><b>B</b></span></div><br><br></div>
    `

    document.getElementById("contenedorliteral").innerHTML = `
    
    <table class="compliance_tab">
    <tbody>

    <tr>
    <td class="letranegro Tipografia--general General--Cuerpo"
        style="background-color:#000020;color:white;text-align:center" colspan="3">Certificaciones
        </td>
</tr>

    <tr class="border_compliance_tabla">
    <td class="letranegro Tipografia--general General--Cuerpo"
        style="width:33.33%;text-align:center"><b style="color:#000020">BRC</b></td>
    <td class="letranegro Tipografia--general General--Cuerpo"
        style="width:765px;text-align:justify;">
        <p style="text-align: justify;padding:5px;width:765px;">La norma BRC es reconocida por la Iniciativa mundial de Seguridad Alimentaria (GFSI), programa destinado a armonizar las normas internacionales de seguridad alimentaria, velando que cumplan con los requisitos que garantizan la seguridad, calidad, y legalidad de sus alimentos.
            <span>Para más información</span> <a href="https://www.normas-iso.com/brc-certificacion-global-seguridad-alimentaria/"
                style="color:rgba(2 146 238 /99%);">clic aquí</a></p>
    </td>
    <td class="letranegro Tipografia--general General--Cuerpo"
        style="width:100%;text-align:center"><img src="img/BCR_IMAGE.png" alt=""
            style="width:38%;"></td>
</tr>

</tbody>
</table>
  

    `
}


function filtroc(){
    
    
    document.getElementById("contenedorletrap").innerHTML = `
    <div class="divfooter2" style="text-align:center;"><span style="font-size:55px;border-bottom:3px solid #262137;"><b>C</b></span></div><br><br></div>
    `
    
    document.getElementById("contenedorliteral").innerHTML = `
    
    
    `;
    
}

function filtrod(){
    
    document.getElementById("contenedorletrap").innerHTML = `
    <div class="divfooter2" style="text-align:center;"><span style="font-size:55px;border-bottom:3px solid #262137;"><b>D</b></span></div><br><br></div>
    `
    
    document.getElementById("contenedorliteral").innerHTML = `
   
    
    `;
    
}


function filtroe(){
    
    document.getElementById("contenedorletrap").innerHTML = `
    <div class="divfooter2" style="text-align:center;"><span style="font-size:55px;border-bottom:3px solid #262137;"><b>E</b></span></div><br><br></div>
    `
    
    document.getElementById("contenedorliteral").innerHTML = `

    <table>
        <tbody>

        <tr>
                        <td class="letranegro Tipografia--general General--Cuerpo"
                            style="background-color:#000020;color:white;text-align:center" colspan="3">Certificaciones</td>
                    </tr>

                    <tr class="border_compliance_tabla">
                    <td class="letranegro Tipografia--general General--Cuerpo"
                        style="width:33.33%;text-align:center"><b style="color:#000020">Ecocert</b></td>
                    <td class="letranegro Tipografia--general General--Cuerpo"
                        style="width:765px;text-align:justify">
                        <p style="text-align: justify;padding:5px;width:765px;">El Certificado ECOCERT es la garantía para un producto en que cumpla con todos los requisitos de sostenibilidad, desde la producción de sus materias primas hasta el producto terminado. <span>Para más información</span> <a
                                href="https://www.ecocert.com/es-MX/quienes-somos"
                                style="color:rgba(2 146 238 /99%);">clic aquí</a></p>
                    </td>
                    <td class="letranegro Tipografia--general General--Cuerpo"
                        style="width:33.33%;text-align:center"><img src="img/ECO-CERT_IMAGE.png" alt=""
                            style="width:50%;"></td>
                </tr>
</table>
</tbody>

    
    `;
    
}

function filtrof(){
    document.getElementById("contenedorletrap").innerHTML = `
    <div class="divfooter2" style="text-align:center;"><span style="font-size:55px;border-bottom:3px solid #262137;"><b>F</b></span></div><br><br></div>
    
 
    
    `;
    document.getElementById("contenedorliteral").innerHTML = `
    
   
    <table>
    <tbody>

    <tr>
                    <td class="letranegro Tipografia--general General--Cuerpo"
                        style="background-color:#000020;color:white;text-align:center" colspan="3">Certificaciones</td>
                </tr>

                <tr class="border_compliance_tabla">
                <td class="letranegro Tipografia--general General--Cuerpo"
                    style="width:33.33%;text-align:center"><b style="color:#000020">FCC</b></td>
                <td class="letranegro Tipografia--general General--Cuerpo"
                    style="width:765px;text-align:justify;">
                    <p style="text-align: justify;padding:5px;width:765px;">El Códice de Sustancias Químicas para Alimentos o Food Chemicals Codex (FCC) es una colección de estándares internacionalmente reconocidos para la pureza e identidad de los ingredientes alimentarios. <span>Para más
                            información</span> <a href="https://www.foodchemicalscodex.org/"
                            style="color:rgba(2 146 238 /99%);">clic aquí</a></p>
                </td>
            
            
                <td class="letranegro Tipografia--general General--Cuerpo"
                    style="width:33.33%;text-align:center"><img
                        src="img/FCC_IMAGE.png" alt="" style="width:50%;">
                </td>
            </tr>
            
            <tr class="border_compliance_tabla">
            <td class="letranegro Tipografia--general General--Cuerpo"
                style="width:33.33%;text-align:center"><b style="color:#000020">FSSC 22000</b></td>
            <td class="letranegro Tipografia--general General--Cuerpo"
                style="width:765px;text-align:justify;">
                <p style="text-align: justify;padding:5px;width:765px;">La Norma FSSC 22000 establece un marco de referencia para los requisitos de la seguridad alimentaria incorporando buenas prácticas en los sistemas agrícolas, de fabricación y distribución, y considerando los principios para el análisis de peligros y puntos críticos de control. <span>Para más
                        información</span> <a href="https://www.dnv.com.mx/services/fssc-22000-certificacion-en-seguridad-alimentaria-5161#:~:text=%C2%BFQu%C3%A9%20es%20la%20FSSC%2022000,para%20la%20fabricaci%C3%B3n%20de%20envases.php"
                        style="color:rgba(2 146 238 /99%);">clic aquí</a></p>
            </td>
            <td class="letranegro Tipografia--general General--Cuerpo"
                style="width:33.33%;text-align:center"><img src="img/FSSC-22000_IMAGE.jpg" alt=""
                    style="width:50%;"></td>
            </tr>

            </tbody>
            </table>

    `;
}


function filtrog(){
    
    document.getElementById("contenedorletrap").innerHTML = `
    <div class="divfooter2" style="text-align:center;"><span class="inicial--productos Tipografia--general"><b>G</b></span></div><br><br></div>
    `
    document.getElementById("contenedorliteral").innerHTML = `
   
    <table>
    <tbody>

    <tr>
                    <td class="letranegro Tipografia--general General--Cuerpo"
                        style="background-color:#000020;color:white;text-align:center" colspan="3">Certificaciones</td>
                </tr>

                <tr class="border_compliance_tabla">
            <td class="letranegro Tipografia--general General--Cuerpo"
                style="width:33.33%;text-align:center"><b style="color:#000020">GMO</b></td>
            <td class="letranegro Tipografia--general General--Cuerpo"
                style="width:765px;text-align:justify;">
                <p style="text-align: justify;padding:5px;width:765px;">Los Organismos Modificados Genéticamente (GMO) son plantas y animales creados en los laboratorios al insertar genes de una especie en la secuencia de ADN de otra especie <span>Para más información</span> <a
                        href="https://www.genome.gov/es/genetics-glossary/Genetically-Modified-Organism"
                        style="color:rgba(2 146 238 /99%);">clic aquí</a></p>
            </td>
            <td class="letranegro Tipografia--general General--Cuerpo"
                style="width:33.33%;text-align:center"><img src="img/GMO_NEW_LOGO.jpg" alt=""
                    style="width:50%;"></td>
        </tr>


      
        <tr class="border_compliance_tabla">
            <td class="letranegro Tipografia--general General--Cuerpo"
                style="width:33.33%;text-align:center"><b style="color:#000020">GMP ICH Q7</b></td>
            <td class="letranegro Tipografia--general General--Cuerpo"
                style="width:765px;text-align:justify">
                <p style="text-align: justify;padding:5px;width:765px;">Buenas prácticas de fabricación de principios activos farmacéuticos
                (Good ManufacturingPractices for Active Pharmaceutical Ingredients) es un documento que proporciona orientación sobrelas buenas prácticas de fabricación para la fabricación de principios activos farmacéuticos (API porsus siglas en inglés) bajo un sistema adecuado de gestión de la calidad. De mismo modo pretendeayudar a garantizar que los API cumplan los requisitos de calidad y pureza.<span>Para más
                        información</span> <a
                        href="Proximamente.php"
                        style="color:rgba(2 146 238 /99%);">clic aquí</a></p>
            </td>
            <td class="letranegro Tipografia--general General--Cuerpo"
                style="width:33.33%;text-align:center"><img src="img/CB_ICH_Q7_271123.jpg" alt=""
                    style="width:70%;"></td>
        </tr>

        </table>
        </tbody>
   
    `
};


function filtroh(){
    document.getElementById("contenedorletrap").innerHTML = `
    
    <div class="divfooter2" style="text-align:center;"><span class="inicial--productos Tipografia--general"><b>H</b></span></div><br><br></div>
    `;
    document.getElementById("contenedorliteral").innerHTML = `

    <table>
    <tbody>
    
    <tr>
                    <td class="letranegro Tipografia--general General--Cuerpo"
                        style="background-color:#000020;color:white;text-align:center" colspan="3">Certificaciones</td>
                </tr>

                <tr class="border_compliance_tabla">
                <td class="letranegro Tipografia--general General--Cuerpo"
                    style="width:33.33%;text-align:center"><b style="color:#000020">HACCP</b></td>
                <td class="letranegro Tipografia--general General--Cuerpo"
                    style="width:765px;text-align:justify;">
                    <p style="text-align: justify;padding:5px;width:765px;">El Sistema de Análisis de Peligros y Control de Puntos Críticos (HACCP - Hazard Análisis and Critical Control Points) el cual permite identificar peligros específicos y medidas para su control con el fin de garantizar la inocuidad de los alimentos. <span>Para más información</span> <a
                            href="https://haccp-international.com/" style="color:rgba(2 146 238 /99%);">clic aquí</a></p>
                </td>
                <td class="letranegro Tipografia--general General--Cuerpo"
                    style="width:33.33%;text-align:center"><img src="img/HACCP_NEW_LOGO.jpg" alt="" style="width:50%;"></td>
            </tr>
    
            <tr class="border_compliance_tabla">
                <td class="letranegro Tipografia--general General--Cuerpo"
                    style="width:33.33%;text-align:center"><b style="color:#000020">HALAL</b></td>
                <td class="letranegro Tipografia--general General--Cuerpo"
                    style="width:765px;text-align:justify;">
                    <p style="text-align: justify;padding:5px;width:765px;">La Certificación Halal determina que un producto o servicio cumple con lo que establece Ley Islámica (Shaira) para que este sea consumido, esto garantiza que los productos agroalimentarios cumplen con el conjunto de prácticas permitidas por la religión musulmana. El término Halal se refiere a todo aquello que está permitido, aquello que es benéfico y saludable para el ser humano ya que aporta mejoras en la calidad de vida y reduce riesgos para la salud.
                        <span>Para más información</span> <a href="https://www.institutohalal.com/que-es-halal/"
                            style="color:rgba(2 146 238 /99%);">clic aquí</a></p>
                </td>
                <td class="letranegro Tipografia--general General--Cuerpo"
                    style="width:33.33%;text-align:center"><img src="img/HALAL_IMAGE.png" alt=""
                        style="width:50%;"></td>
            </tr>
    `;
}

function filtroi(){
   document.getElementById("contenedorletrap").innerHTML = `
    <div class="divfooter2" style="text-align:center;"><span style="font-size:55px;border-bottom:3px solid #262137;"><b>I</b></span></div><br><br></div>
    `;
    document.getElementById("contenedorliteral").innerHTML = `
    
    <table>
    <tbody>

    <tr>
                    <td class="letranegro Tipografia--general General--Cuerpo"
                        style="background-color:#000020;color:white;text-align:center" colspan="3">Certificaciones</td>
                </tr>

                <tr class="border_compliance_tabla">
                <td class="letranegro Tipografia--general General--Cuerpo"
                    style="width:33.33%;text-align:center"><b style="color:#000020">ISO 27001</b></td>
                <td class="letranegro Tipografia--general General--Cuerpo"
                    style="width:765px;text-align:justify">
                    <p style="text-align: justify;padding:5px;width:765px;">Sistema de Gestión de Seguridad de la Información
                    establece parámetros de administración dela información a través del conjunto de políticas, procedimientos y directrices, apalancado de recursosy actividades asociados, que son administrados colectivamente por una empresa u organización,prioritariamente busca proteger sus activos de información esenciales.<span>Para más información</span> <a
                            href="Proximamente.php"
                            style="color:rgba(2 146 238 /99%);">clic aquí</a></p>
                </td>
                <td class="letranegro Tipografia--general General--Cuerpo"
                    style="width:33.33%;text-align:center"><img src="img/iso-27001_IMAGE.png" alt=""
                        style="width:80%;"></td>
            </tr>
    
           
          
            
            
            <tr class="border_compliance_tabla">
                <td class="letranegro Tipografia--general General--Cuerpo"
                    style="width:33.33%;text-align:center"><b style="color:#000020">ISO 37001</b></td>
                <td class="letranegro Tipografia--general General--Cuerpo"
                    style="width:765px;text-align:justify;">
                    <p style="text-align: justify;padding:5px;width:765px;">Sistema de Gestión Antisoborno
                    es un sistema que permite prevenir, detectar y mitigarcualquier intento o acto de soborno. El sistema busca establecer, implementar, mantener y mejorarcontinuamente los elementos que lleven a mitigar el riesgo de soborno en las empresas uorganizaciones.
                        <span>Para más información</span> <a href="Proximamente.php"
                            style="color:rgba(2 146 238 /99%);">clic aquí</a></p>
                </td>
                <td class="letranegro Tipografia--general General--Cuerpo"
                    style="width:33.33%;text-align:center"><img src="img/ISO-37001_IMAGE.jpg" alt=""
                        style="width:70%;"></td>
            </tr>
    
            
            <tr class="border_compliance_tabla">
            <td class="letranegro Tipografia--general General--Cuerpo"
                style="width:33.33%;text-align:center"><b style="color:#000020">ISO 37301</b></td>
            <td class="letranegro Tipografia--general General--Cuerpo"
                style="width:765px;text-align:justify;">
                <p style="text-align: justify;padding:5px;width:765px;">LUn
                Sistema de Gestión de Compliance
                se compone de todas las medidas, estructuras y procesosque una empresa u organización debe establecer para garantizar el cumplimiento de las normasaplicables y la ley vigente. De mismo modo, debe ser conocido y apoyado por la alta dirección, paramantener desde todas las áreas la identidad e integridad corporativa.
                    <span>Para más información</span> <a href="Proximamente.php"
                        style="color:rgba(2 146 238 /99%);">clic aquí</a></p>
            </td>
            <td class="letranegro Tipografia--general General--Cuerpo"
                style="width:33.33%;text-align:center"><img src="img/ISO-37301_IMAGE.jpg" alt=""
                    style="width:100%;"></td>
        </tr>
    
        <tr class="border_compliance_tabla">
        <td class="letranegro Tipografia--general General--Cuerpo"
            style="width:33.33%;text-align:center"><b style="color:#000020">ISO 9001</b></td>
        <td class="letranegro Tipografia--general General--Cuerpo"
            style="width:765px;text-align:justify;">
            <p style="text-align: justify;padding:5px;width:765px;">La Certificación ISO 9001:2015 es una norma internacional de gestión de la calidad aplicable a cualquier tipo de organización en cualquier sector o actividad empresarial o industrial. Está basada en los siete principios de gestión de calidad, fundamentales para una buena gestión empresarial. 
                <span>Para más información</span> <a href="https://www.iso.org/obp/ui/#iso:std:iso:9001:ed-4:v2:es"
                    style="color:rgba(2 146 238 /99%);">clic aquí</a></p>
        </td>
        <td class="letranegro Tipografia--general General--Cuerpo"
            style="width:33.33%;text-align:center"><img src="img/ISO-9001_IMAGE.jpg" alt=""
                style="width:70%;"></td>
    </tr>

                    </tbody>
                    </table>

    `;
}

// function filtroj(){
//     document.getElementById("contenedorliteral").innerHTML = `
    
//     <img src="img/AGROQUIMICOS2_CB.jpg" alt="" style="width:100%;">
//     `;
//     document.getElementById("contenedorliteral2").innerHTML = `
    
//     <h5>Bakes</h5>
//     <hr style="border:1px solid #b21b27">
//     <p> Los acidos son un elemento fundamental agentes usados usados por una gran variedad de industrias para remover impurezas y producir fertilizantes, incluso pintura para los ojos</p>
//     <b><a href="#" style="color:#212637;" class="arrowblue">Leer Mas...</a></b>
//     `;
// }

function filtroj(){
    document.getElementById("contenedorletrap").innerHTML = `
    <div class="divfooter2" style="text-align:center;"><span class="inicial--productos Tipografia--general"><b>J</b></span></div><br><br></div>
    `
    document.getElementById("contenedorliteral").innerHTML = `
    
    <table>
    <tbody>

    <tr>
                    <td class="letranegro Tipografia--general General--Cuerpo"
                        style="background-color:#000020;color:white;text-align:center" colspan="3">Marco
                        Regulatorio</td>
                </tr>

                <tr class="border_compliance_tabla">
                        <td class="letranegro Tipografia--general General--Cuerpo"
                            style="width:33.33%;text-align:center"><b style="color:#000020">JECFA</b></td>
                        <td class="letranegro Tipografia--general General--Cuerpo"
                            style="width:765px;text-align:justify">
                            <p style="text-align: justify;padding:5px;width:765px;">El Comité Mixto FAO/OMS de
                                Expertos en Aditivos Alimentarios (JECFA) es el órgano internacional encargado de la
                                evaluación de la inocuidad de los aditivos alimentarios. <span>Para más
                                    información</span> <a
                                    href="https://www.fao.org/food/food-safety-quality/scientific-advice/jecfa/es/"
                                    style="color:rgba(2 146 238 /99%);">clic aquí</a></p>
                        </td>
                        <td class="letranegro Tipografia--general General--Cuerpo"
                            style="width:33.33%;text-align:center"><img src="img/JECFA_IMAGE.jpg" alt=""
                                style="width:50%;"></td>
                    </tr>

                    <tr class="border_compliance_tabla">
                        <td class="letranegro Tipografia--general General--Cuerpo"
                            style="width:33.33%;text-align:center"><b style="color:#000020">JP</b></td>
                        <td class="letranegro Tipografia--general General--Cuerpo"
                            style="width:765px;text-align:justify;">
                            <p style="text-align: justify;padding:5px;width:765px;">La
                                Farmacopea Japonesa
                                (o Japaneese Pharmacopoeia) es la farmacopea oficial del estado delJapón. Es editada
                                por la Agencia de dispositivos médicos y productos farmacéuticos bajo la
                                autoridaddel Ministerio de sanidad, trabajo y bienestar de Japón, es por dicho
                                motivo que mantiene un altogrado de exigencia. <span>Para más información</span> <a
                                    href="Proximamente.php" style="color:rgba(2 146 238 /99%);">clic aquí</a></p>
                        </td>
                        <td class="letranegro Tipografia--general General--Cuerpo"
                            style="width:33.33%;text-align:center"><img src="img/" alt="" style="width:50%;"></td>
                    </tr>


   
    `
}


function filtrok(){
    document.getElementById("contenedorletrap").innerHTML = `
    
    <div class="divfooter2" style="text-align:center;"><span style="font-size:55px;border-bottom:3px solid #262137;"><b>K</b></span></div><br><br></div>
    `;
    document.getElementById("contenedorliteral").innerHTML = `
    
    <table class="compliance_tab">
    <tbody>

    


        <tr>
            <td class="letranegro Tipografia--general General--Cuerpo"
                style="background-color:#000020;color:white;text-align:center" colspan="3">Certificaciones
                </td>
        </tr>

        <tr class="border_compliance_tabla">
<td class="letranegro Tipografia--general General--Cuerpo"
    style="width:33.33%;text-align:center"><b style="color:#000020">KOSHER</b></td>
<td class="letranegro Tipografia--general General--Cuerpo"
    style="width:765px;text-align:justify;">
    <p style="text-align: justify;padding:5px;width:765px;">La Certificación Kosher es aquella que indica que dichos productos respetan los preceptos de la religión judía y que, por tanto, se consideran puros y aptos para ser ingeridos por los practicantes de dicha religión.
        <span>Para más información</span> <a href="https://oukosher.org/espanol/que-es-el-kosher/"
            style="color:rgba(2 146 238 /99%);">clic aquí</a></p>
</td>
<td class="letranegro Tipografia--general General--Cuerpo"
    style="width:33.33%;text-align:center"><img src="img/KOSHER_IMAGE.jpg" alt=""
        style="width:50%;"></td>
</tr>

    `;
}


function filtrol(){
    
    
    document.getElementById("contenedorletrap").innerHTML = `
    <div class="divfooter2" style="text-align:center;"><span class="inicial--productos Tipografia--general">L</span></div><br><br></div>
    `
    
    document.getElementById("contenedorliteral").innerHTML = `


  
    `;
}

function filtrom(){
    
    document.getElementById("contenedorletrap").innerHTML = `
    <div class="divfooter2" style="text-align:center;"><span class="inicial--productos Tipografia--general">M</span></div><br><br></div>
    `
    
    document.getElementById("contenedorliteral").innerHTML = `


   
    `;
}

function filtron(){

    document.getElementById("contenedorletrap").innerHTML = `
    <div class="divfooter2" style="text-align:center;"><span class="inicial--productos Tipografia--general"><b>N</b></span></div><br><br></div>
    `
    document.getElementById("contenedorliteral").innerHTML = `
    
    <table>
    <tbody>

    <tr>
    <td class="letranegro Tipografia--general General--Cuerpo"
        style="background-color:#000020;color:white;text-align:center" colspan="3">Certificaciones</td>
</tr>

    <tr class="border_compliance_tabla">
<td class="letranegro Tipografia--general General--Cuerpo"
style="width:33.33%;text-align:center"><b style="color:#000020">NON-GMO</b></td>
<td class="letranegro Tipografia--general General--Cuerpo"
style="width:765px;text-align:justify;">
<p style="text-align: justify;padding:5px;width:765px;">La certificación NON GMO evalúa alimentos de cuyos ingredientes o algún algún proceso de la creación de ellos, se ocupen organismos modificados genéticamente. Dicho de otro modo, certifica productos alimenticios completamente orgánicos, con todas sus cualidades naturales intactas.
    <span>Para más información</span> <a href="Proximamente.php"
        style="color:rgba(2 146 238 /99%);">clic aquí</a></p>
</td>
<td class="letranegro Tipografia--general General--Cuerpo"
style="width:33.33%;text-align:center"><img src="img/NON-GMO_IMAGE.jpg" alt=""
    style="width:50%;"></td>
</tr>
            </tbody>
            </table>
   

    `
}

function filtroo(){
    document.getElementById("contenedorliteral").innerHTML = `
    
    <img src="img/AGROQUIMICOS2_CB.jpg" alt="" style="width:100%;">
    `;
    document.getElementById("contenedorliteral2").innerHTML = `
    
    <h5>Bakes</h5>
    <hr style="border:1px solid #b21b27">
    <p> Los ácidos son un elemento fundamental agentes usados usados por una gran variedad de industrias para remover impurezas y producir fertilizantes, incluso pintura para los ojos</p>
    <b><a href="#" style="color:#212637;" class="arrowblue">Leer Mas...</a></b>
    `;
}


function filtrop(){
    document.getElementById("contenedorletrap").innerHTML = `
    <div class="divfooter2" style="text-align:center;"><span class="inicial--productos Tipografia--general"><b>P</b></span></div><br><br></div>
    `
    document.getElementById("contenedorliteral").innerHTML = `
    
    
    <table>
    <tbody>

    <tr>
    <td class="letranegro Tipografia--general General--Cuerpo"
        style="background-color:#000020;color:white;text-align:center" colspan="3">Certificaciones</td>
</tr>
    
<tr class="border_compliance_tabla">
<td class="letranegro Tipografia--general General--Cuerpo"
style="width:33.33%;text-align:center"><b style="color:#000020">PAREVE</b></td>
<td class="letranegro Tipografia--general General--Cuerpo"
style="width:765px;text-align:justify;">
<p style="text-align: justify;padding:5px;width:765px;">La certificación
Kosher PAREVE
(NEUTRO) cumple al pie de la letra con las exigencias de aquellaspersonas que por diversas razones requieren de una dieta vegetariana, vegana o muy estricta, y queno pueden contener ningún tipo de grasa animal, debido a que esta certificación asegura lainexistencia de contaminación o arrastre de partículas que afectan a personas con diferentes tipos dealergias, principalmente de derivados de lácteos o cárnicos.
<span>Para más información</span> <a href="https://ukkosher.org/empresas/carnicos-lacteos-y-pareve/#:~:text=Los%20alimentos%20que%20no%20son,caf%C3%A9%2C%20t%C3%A9%20y%20diversos%20dulces."
    style="color:rgba(2 146 238 /99%);">clic aquí</a></p>
</td>
<td class="letranegro Tipografia--general General--Cuerpo"
style="width:33.33%;text-align:center"><img src="img/CB_PAREVE_271123.png" alt=""
style="width:50%;"></td>
</tr>


    `
}


function filtroq(){
    document.getElementById("contenedorliteral").innerHTML = `
    
    <img src="img/AGROQUIMICOS2_CB.jpg" alt="" style="width:100%;">
    `;
    document.getElementById("contenedorliteral2").innerHTML = `
    
    <h5>Bakes</h5>
    <hr style="border:1px solid #b21b27">
    <p> Los ácidos son un elemento fundamental agentes usados usados por una gran variedad de industrias para remover impurezas y producir fertilizantes, incluso pintura para los ojos</p>
    <b><a href="#" style="color:#212637;" class="arrowblue">Leer Mas...</a></b>
    `;
}

function filtror(){
    document.getElementById("contenedorletrap").innerHTML = `
    
    <div class="divfooter2" style="text-align:center;"><span style="font-size:55px;border-bottom:3px solid #262137;"><b>R</b></span></div><br><br></div>
    `;
    document.getElementById("contenedorliteral").innerHTML = `
    
    <table>
    <tbody>

    <tr>
                    <td class="letranegro Tipografia--general General--Cuerpo"
                        style="background-color:#000020;color:white;text-align:center" colspan="3">Marco
                        Regulatorio</td>
                </tr>

                
                <tr class="border_compliance_tabla">
                <td class="letranegro Tipografia--general General--Cuerpo"
                    style="width:33.33%;text-align:center"><b style="color:#000020">REACH</b></td>
                <td class="letranegro Tipografia--general General--Cuerpo"
                    style="width:765px;text-align:justify">
                    <p style="text-align: justify;padding:5px;width:765px;">El REACH (Registro, Evaluación,
                        Autorización y Restricción de las Sustancias y Mezclas Químicas / Registration,
                        Evaluation, Authorisation and Restriction of Chemicals) es un Reglamento de la Unión
                        Europea Reglamento (CE) nº 1907/2006, adoptado con el fin de mejorar la protección
                        de la salud humana y el medio ambiente contra los riesgos que pueden presentar los
                        productos químicos. <span>Para más información</span> <a
                            href="https://osha.europa.eu/es/themes/dangerous-substances/reach"
                            style="color:rgba(2 146 238 /99%);">clic aquí</a></p>
                </td>
                <td class="letranegro Tipografia--general General--Cuerpo"
                    style="width:33.33%;text-align:center"><img src="img/REACH-1_IMAGE.jpg" alt=""
                        style="width:50%;"></td>
            </tr>



    `;
}

function filtros(){
    
    document.getElementById("contenedorletrap").innerHTML = `
    
     <div class="divfooter2" style="text-align:center;"><span style="font-size:55px;border-bottom:3px solid #262137;"><b>S</b></span></div><br><br></div>
    
    `
    
    document.getElementById("contenedorliteral").innerHTML = `

    <table>
    <tbody>

    <tr>
    <td class="letranegro Tipografia--general General--Cuerpo"
        style="background-color:#000020;color:white;text-align:center" colspan="3">Certificaciones</td>
</tr>

    
<tr class="border_compliance_tabla">
<td class="letranegro Tipografia--general General--Cuerpo"
style="width:33.33%;text-align:center"><b style="color:#000020">SQF</b></td>
<td class="letranegro Tipografia--general General--Cuerpo"
style="width:765px;text-align:justify;">
<p style="text-align: justify;padding:5px;width:765px;">El programa SQF (Safe Quality Food / Seguridad de Calidad Alimentaria) es un programa de calidad e inocuidad de los alimentos riguroso y confiable con reconocimiento por parte de minoristas, propietarios de marcas y proveedores de servicios de alimentos a nivel mundial.
<span>Para más información</span> <a href="https://www.sqfi.com/es-mx/our-program/what-is-sqf/noriko-nakama"
style="color:rgba(2 146 238 /99%);">clic aquí</a></p>
</td>
<td class="letranegro Tipografia--general General--Cuerpo"
style="width:33.33%;text-align:center"><img src="img/SQF_IMAGE.png" alt=""
style="width:50%;"></td>
</tr>
    
    `;
    
    
}

function filtrot(){
    
    document.getElementById("contenedorletrap").innerHTML = `
    
    <div class="divfooter2" style="text-align:center;"><span style="font-size:66px;">S</span></div><br><br></div>
    
    
    `
    
    document.getElementById("contenedorliteral").innerHTML = `
    
    <div class = "col s12 m12 l12 xl12 center-align" style="margin-top:30px">
    
    <img src="img/SORBITOL_280721.jpg" alt="" style="width:100%;">
    
    </div>
    
    
    
    `;
    
    document.getElementById("contenedorliteral2").innerHTML = `
    
    
    
    
    `
}

function filtrou(){
    
    document.getElementById("contenedorletrap").innerHTML = `
    
    <div class="divfooter2" style="text-align:center;"><span style="font-size:55px;border-bottom:3px solid #262137;"><b>U</b></span></div><br><br></div>
    
    
    `
    
    document.getElementById("contenedorliteral").innerHTML = `
    
    <table>
    <tbody>

    <tr>
                    <td class="letranegro Tipografia--general General--Cuerpo"
                        style="background-color:#000020;color:white;text-align:center" colspan="3">Marco
                        Regulatorio</td>
                </tr>

                <tr class="border_compliance_tabla">
                        <td class="letranegro Tipografia--general General--Cuerpo"
                            style="width:33.33%;text-align:center"><b style="color:#000020">USP</b></td>
                        <td class="letranegro Tipografia--general General--Cuerpo"
                            style="width:765px;text-align:justify;">
                            <p style="text-align: justify;padding:5px;width:765px;">La USP por sus siglas en inglés
                                (
                                United States Pharmacopoeia
                                ), o bien, Farmacopea de los EstadosUnidos de América, establece estándares y
                                parámetros para los ingredientes de los alimentos y suplementos alimenticios.
                                <span>Para más información</span> <a href="Proximamente.php"
                                    style="color:rgba(2 146 238 /99%);">clic aquí</a></p>
                        </td>
                        <td class="letranegro Tipografia--general General--Cuerpo"
                            style="width:33.33%;text-align:center"><img src="img/USP_LOGO.png" alt=""
                                style="width:50%;"></td>
                    </tr>

                    </tbody>
                    </table>
    
    
    
    `;
    
    document.getElementById("contenedorliteral2").innerHTML = `
    
    
 
    
    `
}

function filtrov(){
    
    document.getElementById("contenedorletrap").innerHTML = `
    
    <div class="divfooter2" style="text-align:center;"><span style="font-size:66px;">S</span></div><br><br></div>
    
    
    `
    
    document.getElementById("contenedorliteral").innerHTML = `
    
    <div class = "col s12 m12 l12 xl12 center-align" style="margin-top:30px">
    
    <img src="img/SORBITOL_280721.jpg" alt="" style="width:100%;">
    
    </div>
    
    
    
    `;
    
    document.getElementById("contenedorliteral2").innerHTML = `
    
    

    
    `
}

function filtrow(){
    
    document.getElementById("contenedorletrap").innerHTML = `
    
    <div class="divfooter2" style="text-align:center;"><span style="font-size:66px;">S</span></div><br><br></div>
    
    
    `
    
    document.getElementById("contenedorliteral").innerHTML = `
    
    <table class="compliance_tab">
    <tbody>

    
    
    
    `;
    
    document.getElementById("contenedorliteral2").innerHTML = `
    
    
    <div class = "col s12 m12 l12 xl12">
    <h5><b>Sorbitols</b></h5>
    <hr style="border:1px solid #b21b27">
    <p style="text-align:justify"> El sorbitol se produce mediante la hidrogenación catalítica de D-glucosa y está disponible como un polvo cristalino de flujo libre y como soluciones acuosas. 
    
    Nuestro sorbitol cristalino es un excipiente único para la compresión directa de comprimidos. El sorbitol tiene excelentes propiedades de flujo, lo que garantiza una mezcla uniforme y un peso de tableta constante. 
    </p>
    <b><a href="Cargil_Sorbitols.php" style="color:#212637;" class="arrowblue">Leer Mas...</a></b>
    </div>
    
    `
}


function filtrox(){
    
    document.getElementById("contenedorletrap").innerHTML = `
    
    <div class="divfooter2" style="text-align:center;"><span style="font-size:66px;">S</span></div><br><br></div>
    
    `
    document.getElementById("contenedorliteral").innerHTML = `
    
    <div class = "col s12 m12 l12 xl12 center-align" style="margin-top:30px">
    
    <img src="img/SORBITOL_280721.jpg" alt="" style="width:100%;">
    
    </div>
    
    
    
    `;
    
    document.getElementById("contenedorliteral2").innerHTML = `
    
    
  
    
    `
}



function filtroy(){
    
    document.getElementById("contenedorletrap").innerHTML = `
    
    <div class="divfooter2" style="text-align:center;"><span style="font-size:66px;">S</span></div><br><br></div>
    
    
    `
    
    document.getElementById("contenedorliteral").innerHTML = `
    
    <div class = "col s12 m12 l12 xl12 center-align" style="margin-top:30px">
    
    <img src="img/SORBITOL_280721.jpg" alt="" style="width:100%;">
    
    </div>
    
    
    
    `;
    
    document.getElementById("contenedorliteral2").innerHTML = `
    
    

    `
}



function filtroz(){
    
    document.getElementById("contenedorletrap").innerHTML = `
    
    <div class="divfooter2" style="text-align:center;"><span style="font-size:66px;">S</span></div><br><br></div>
    
    
    `
    
    document.getElementById("contenedorliteral").innerHTML = `
    
    <div class = "col s12 m12 l12 xl12 center-align" style="margin-top:30px">
    
    <img src="img/SORBITOL_280721.jpg" alt="" style="width:100%;">
    
    </div>
    `;
    
    document.getElementById("contenedorliteral2").innerHTML = `
    
    

    `
}

function mostrartexto(){
    
    document.getElementById("textomostrar").style.display = "block";
    
    document.getElementById("textomostrar1").style.display = "block";
    
    document.getElementById("readm").style.display = "none";
    
    document.getElementById("readl").style.display = "inline";
    
}

function ocultartexto(){
    
    document.getElementById("textomostrar").style.display = "none";
    
    document.getElementById("textomostrar1").style.display = "none";
    
    document.getElementById("readl").style.display = "none";
    
    document.getElementById("readm").style.display = "inline";
    
}




